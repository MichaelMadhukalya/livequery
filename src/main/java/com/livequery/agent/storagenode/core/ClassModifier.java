...
/**
 * <p>
 * The purpose of <code>ClassModifier</code> is to overwrite byte codes for a
 * given input class on disk with updated fields/methods/annotations. Once the
 * byte codes are overwritten, the app class loader can re-load the class again
 * with updated fields/methods.
 * </p>
 *
 * <p>
...
public class ClassModifier {
    ...
    public boolean write(byte[] data) {
        ...
        if (null == path || StringUtils.isEmpty(path.toString())) {
            logger.error(String.format("Unable to locate class %s inside class path", type.getSimpleName()));
            return false;
        }
        ...
        try (FileOutputStream fileOutputStream = new FileOutputStream(
            ...
            success = true;
            logger.info(String.format("Successfully updated file : {%s}", path.toAbsolutePath().toString()));
        } catch (Exception e) {
    ...
    /**
     * Finds the location of a class file by searching inside class path. The class path is provided as a property inside
    ...
    private Path findClass(String classpath) {
        ...
        try {
            Files.walkFileTree(Paths.get(classpath), new FileVisitor<Path>() {
                ...
                public FileVisitResult visitFile(Path path, BasicFileAttributes basicFileAttributes) throws IOException {
                    ...
                    if (path.toString().contains(classFile)) {
                        location = path;
                        logger.info(String.format("Class {%s} found inside classpath with file name {%s}",
                ...
                public FileVisitResult visitFileFailed(Path path, IOException e) throws IOException {
                    if (null != e) {
                        logger.error(String.format("File visit failed %s with exception {%s}", path.toString(), e));
                    }
                ...
                public FileVisitResult postVisitDirectory(Path path, IOException e)
                    throws IOException {
                    if (null != e) {
                        logger.error(String.format("Directory visit failed %s with exception {%s}", path.toString(), e));
                    }
            ...
            logger.error(
                String.format("Exception while trying to search for class %s in classpath {%s}", type.getSimpleName(), e));
        }
