...
/**
 * <p>
 * <code>ClassVerify</code> will verify that a class can be loaded and its
 * fields, methods and annotations are accessible to caller. The input to this class will be a type token parameterized using a
 * type class whose byte codes were modified by <code>livequery</code> application. A typical caller will call verify after the
 * parameterized type class has been re-loaded using the application class loader: <code>AppClassLoader</code>
 */
